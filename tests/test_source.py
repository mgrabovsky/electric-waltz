...
class DispatchableTestCase(TestCase):
    def test_init(self):
        ccgt = DispatchableSource(nominal=500, self_consumption=0.2)
    ...
    def test_shut_down(self):
        ccgt = DispatchableSource(nominal=500, self_consumption=0.1)
        self.assertEqual(ccgt.generation, 500)
        self.assertEqual(ccgt.utilisation, 1)
...
class NonDispatchableTestCase(TestCase):
    def test_init(self):
        pve = NonDispatchableSource(nominal=500, self_consumption=0.2)
...
class PowerSourceTestCase(TestCase):
    def test_init_util_zero(self):
        source = PowerSource(nominal=500, self_consumption=0, utilisation=0)
    ...
    def test_init_perfect_util_half(self):
        source = PowerSource(nominal=500, self_consumption=0, utilisation=0.5)
    ...
    def test_init_perfect_util_full(self):
        source = PowerSource(nominal=500, self_consumption=0, utilisation=1)
    ...
    def test_init_imperfect_util_half(self):
        source = PowerSource(nominal=1000, self_consumption=0.1, utilisation=0.5)
        self.assertEqual(source.generation, 500)
        self.assertEqual(source.net_generation, 450)
        self.assertEqual(source.utilisation, 0.5)

    def test_init_imperfect_util_full(self):
        source = PowerSource(nominal=1000, self_consumption=0.1, utilisation=1)
